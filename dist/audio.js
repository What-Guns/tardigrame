...
;
export function playSoundLooped(buffer) {
    const sound = audioContext.createBufferSource();
    sound.buffer = buffer;
    sound.connect(gainNode);
    sound.loop = true;
...
;
export function playSoundAtLocation(buffer, location) {
    const sound = audioContext.createBufferSource();
    sound.buffer = buffer;
...
export function createSoundLibrary(descriptor) {
    const library = {};
...
export async function createSoundLibraryAsync(descriptor) {
    const library = {};
...
window.fade0 = fadeInBGM0;
window.fade1 = fadeInBGM1;
window.fade2 = fadeInBGM2;
