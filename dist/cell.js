...
export const hydratedCells = new Set();
export const mossyCells = new Set();
export const cellsThatNeedWorkDone = new Set();
...
export class Cell {
    ...
    get hydration() {
        if (this.type !== 'POOL' && this.type !== 'WATER_SOURCE' && this.type !== 'CAPSULE')
            return false;
        return this._hydration;
    }
    set hydration(hydrated) {
        ...
        if (hydrated)
            hydratedCells.add(this);
        else
            hydratedCells.delete(this);
    ...
    consumeMoss(amount) {
        if (this.type !== 'MOSS')
            throw new Error(`No moss to eat on ${this.type}`);
